...
class main{
	...
	// At first we create a random square matrix - let's call it C now to avoid confusion with
	// part 1 above
	...
	// Next we generate a vector b of a length corresponding to the dimension of the square matrix
	...
	var qrGSa = new qrDecompositionGS(C);
	Q = qrGSa.Q;
	R = qrGSa.R;

	vector x = qrGSa.solve(b);
