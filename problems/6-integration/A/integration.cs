...
public partial class integration{
	...
	double f2 = NaN, double f3 = NaN, int fCalls = 0){
		...
		if(error < acc + eps*Abs(Q)){
			return new vector(new double[] {Q, error, fCalls});
		}else{
			...
			// Notice that for recursive calls the counter of function calls has to
			// be divided by two before passing it on, to avoid over-counting already
			// done function evaluations
			vector interval1 = o4a(f, a, (a+b)/2, acc/Sqrt(2), eps, f1, f2, fCalls/2);
			vector interval2 = o4a(f, (a+b)/2, b, acc/Sqrt(2), eps, f3, f4, fCalls/2);
			
			// Add results
			Q = interval1[0] + interval2[0];
			error = Sqrt(Pow(interval1[1], 2) + Pow(interval2[1], 2));
			double fCallsTotal = interval1[2] + interval2[2];
			return new vector(new double[] {Q, error, fCallsTotal});
		}
	}
}
